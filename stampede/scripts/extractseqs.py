...
Read BLAST output file to get sequence hit ids for the reference database. Extract the reference database
...
The second column is parsed to extract the reference database name (e.g. `HOT234_1_0200m`) and the reference
database sequence id (e.g. `rep_c55158_2`). All matched sequences are extracted from the reference databases and
written to a file.
"""
...
def main(blast_output_fp, ohana_sequence_dp, ohana_hit_output_dp, blast_hit_limit):
    ...
    with open(blast_output_fp, 'rt') as blast_output_file:
        blast_db_hits = get_blast_reference_hits(blast_output_file=blast_output_file, blast_hit_limit=blast_hit_limit)
    ...
    blast_db_fasta_file_pattern = os.path.join(
        ohana_sequence_dp,
        '{refdb}',
    ...
    output_file_pattern = os.path.join(
        ohana_hit_output_dp,
        blast_input_base_name + '-{refdb}-' + seq_type + seq_type_ext[seq_type]
    )

    # find each hit from the BLAST output in the corresponding BLAST reference database and copy the sequence to a file
    for blast_db_name, matched_sequence_ids in sorted(blast_db_hits.items()):
        blast_db_fasta_fp = blast_db_fasta_file_pattern.format(refdb=blast_db_name)
        if not os.path.exists(blast_db_fasta_fp):
            print('ERROR: BLAST database "{}" does not exist'.format(blast_db_fasta_fp))
        else:
            output_fp = output_file_pattern.format(refdb=blast_db_name)
            print('extracting sequence hits from "{}"'.format(blast_db_fasta_fp))
            with open(blast_db_fasta_fp, 'rt') as blast_db_fasta_file, open(output_fp, 'wt') as output_file:
                write_sequence_hits(matched_sequence_ids, blast_db_fasta_file, output_file)
